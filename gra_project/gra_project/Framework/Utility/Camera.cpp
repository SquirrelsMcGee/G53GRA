...
Camera::Camera() : wKey(0), sKey(0), aKey(0), dKey(0), plusKey(0), minusKey(0), currentButton(0), mouseX(0), mouseY(0)
{
	Reset();
}
...
void Camera::Reset(){
	...
	/* This section has been disabled intentionally
	eyePosition[0] = 0.0f;
	eyePosition[1] = 0.0f;
...
void Camera::HandleMouseDrag(int x, int y)
{
	...
	rx = static_cast<float>(x - mouseX) / 2.f;
	ry = static_cast<float>(y - mouseY) / 2.f;
...
void Camera::HandleMouseMove(int x, int y)
{
	...
	windowWidth = Scene::GetWindowWidth();
	windowHeight = Scene::GetWindowHeight();
